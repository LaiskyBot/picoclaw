...
- Responses requests that need web search should explicitly include `{"type":"web_search"}` in `tools`, ensuring consistent behavior across channels and compatibility with current OpenAI request validation.
...
- Downstream clients should consume the native Responses envelope directly; search metadata (query, domains, optional sources) is preserved so callers can replay or audit exact web lookups.
...
When constructing a message, you should try and keep content that you expect to use over and over in your API requests at the beginning of your prompt, **and** among the first API parameters you pass in the JSON request body to [Responses](/docs/api-reference/responses). This enables you to maximize cost and latency savings from [prompt caching](/docs/guides/prompt-caching).
...
```bash
BASE64_IMAGE=$(base64 < path_to_your_image.jpg) && curl https://api.openai.com/v1/responses   -H "Content-Type: application/json"   -H "Authorization: Bearer $OPENAI_API_KEY"   -d @- <<EOF
  {
    "model": "gpt-4.1-mini",
    "input": [
      {
...
Structured Outputs is the evolution of [JSON mode](#json-mode). While both ensure valid JSON is produced, only Structured Outputs ensure schema adherance. Both Structured Outputs and JSON mode are supported in the Responses API, Assistants API, Fine-tuning API and Batch API.
...
math_reasoning = completion.output_parsed
...
const math_reasoning = completion.output_parsed;
